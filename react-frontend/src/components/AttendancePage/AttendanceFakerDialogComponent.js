...
import attendanceFakerFactory from "./attendanceFakerFactory";
...
const AttendanceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = attendanceFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("attendance").create(elem));
...
export default connect(null, mapDispatch)(AttendanceFakerDialogComponent);
